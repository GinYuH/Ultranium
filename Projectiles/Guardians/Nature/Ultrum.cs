...
public class Ultrum : ModProjectile
{
	...
	public override void AI()
	{
        ...
        player.AddBuff(ModContent.BuffType<UltrumBuff>(), 3600, quiet: false);
        if (num)
		{
			if (player.dead)
			{
		...
		if (Main.myPlayer == Projectile.owner)
		{
			...
			vector4.Normalize();
			vector4 *= 8.5f;
			int num9 = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, vector4.X, vector4.Y, Mod.Find<ModProjectile>("NatureBlast").Type, Projectile.damage, Projectile.knockBack, Main.myPlayer, 0f, 0f);

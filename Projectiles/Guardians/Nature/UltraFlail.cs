...
public class UltraFlail : ModProjectile
{
	...
	public override void AI()
	{
		...
		{
			Projectile.velocity += 60f * spinningpoint;
		}
		else
		{
			Projectile.Kill();
		...
		if (Projectile.alpha == 0)
		{
			for (int i = 0; i < 2; i++)
			{
				Dust obj = Main.dust[Dust.NewDust(Projectile.position + Projectile.velocity * 2f, Projectile.width, Projectile.height, Mod.Find<ModDust>("UltraniumDust").Type, 0f, 0f, 100, default(Color), 2f)];
				obj.noGravity = true;
				obj.velocity *= 2f;

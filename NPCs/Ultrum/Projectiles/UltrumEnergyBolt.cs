...
public class UltrumEnergyBolt : ModProjectile
{
	...
	public override void OnKill(int timeLeft)
	{
		...
		for (int j = 0; j < 5; j++)
		{
			...
			vector.Normalize();
			vector *= 6f;
			Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, vector.X, vector.Y, Mod.Find<ModProjectile>("UltrumBolt").Type, 30, 1f, Main.myPlayer, 0f, 0f);
		}
	}
}

...
public class CosmicStar : ModProjectile
{
	...
	{
		((ModProjectile)this).projectile.width = 96;
		((ModProjectile)this).projectile.height = 96;
	...
	{
		Main.PlaySound(((ModProjectile)this).mod.GetLegacySoundSlot((SoundType)50, "Sounds/CosmicStarAttack")?.WithVolume(1f), -1, -1);
		...
		for (int j = 0; j < 10; j++)
		{
			Vector2 vector = ((float)Math.PI / 5f * (float)j).ToRotationVector2();
			vector.Normalize();
			vector *= 10f;
			Projectile.NewProjectile(((ModProjectile)this).projectile.Center.X, ((ModProjectile)this).projectile.Center.Y, vector.X, vector.Y, ((ModProjectile)this).mod.ProjectileType("CosmicStarSmall"), ((ModProjectile)this).projectile.damage, 1f, Main.myPlayer, 0f, 0f);
		}
	}
}

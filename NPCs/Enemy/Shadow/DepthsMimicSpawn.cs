...
public class DepthsMimicSpawn : ModPlayer
{
	...
	public static bool ChestItemSummonCheck(int x, int y, Mod mod)
	{
		...
		int num2 = 0;
		int num3 = 0;
		ushort type = Main.tile[Main.chest[num].x, Main.chest[num].y].type;
		int num4 = Main.tile[Main.chest[num].x, Main.chest[num].y].frameX / 36;
		if (type == 21 && (num4 < 5 || num4 > 6))
		{
			for (int i = 0; i < 40; i++)
			{
				...
				{
					if (Main.chest[num].item[i].type == mod.ItemType("DepthsKey"))
					{
		...
		if (num3 == 0 && num2 == 1)
		{
			...
			int num5 = mod.NPCType("DepthsMimic");

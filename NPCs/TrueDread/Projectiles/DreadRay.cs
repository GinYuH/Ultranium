...
public class DreadRay : ModProjectile
{
	...
	{
		((ModProjectile)this).DisplayName.SetDefault("Dread Death Ray");
	}
	...
	public override void AI()
	{
		...
		((ModProjectile)this).projectile.ai[1] += 6f * multiplier;
	...
	public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
	{
		...
		Texture2D texture = ((ModProjectile)this).mod.GetTexture("NPCs/TrueDread/Projectiles/DreadRayBottom");
		Texture2D texture2D = Main.projectileTexture[((ModProjectile)this).projectile.type];
		Texture2D texture2 = ((ModProjectile)this).mod.GetTexture("NPCs/TrueDread/Projectiles/DreadRayTip");
		float laserLength = LaserLength;
		Color color = Color.White * 0.8f * ((((ModProjectile)this).projectile.ai[1] >= 100f) ? 1f : 0.8f);
		...
		if (laserLength > 0f)
		{
			...
			while (num + 1f < laserLength)
			{
				...
				num += (float)value.Height * ((ModProjectile)this).projectile.scale;
